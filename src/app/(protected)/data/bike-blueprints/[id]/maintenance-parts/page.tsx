...
import {
  ...
  type BlueprintMaintenancePartRowRecord,
  type BrandRecord,
        ...
        <div className="grid gap-5 lg:grid-cols-[1.3fr_0.8fr]">
          ...
          <aside className="rounded-[1.5rem] border border-outline-variant/15 bg-surface-container-low p-4">
            ...
            <div className="mt-6 flex flex-col gap-3">
              ...
              >
                {isLoading ? "Saving..." : multiple ? "Assign Selected" : "Assign Maintenance Part"}
              </button>
              <button
                type="button"
                onClick={onClose}
...
export default function BikeBlueprintMaintenancePartsPage() {
  ...
  const [assignedRows, setAssignedRows] = useState<BlueprintMaintenancePartRowRecord[]>([]);
  const [availableParts, setAvailableParts] = useState<MaintenancePartRecord[]>([]);
  ...
  const loadAssignedRows = useCallback(async () => {
    try {
      ...
      const [blueprintResult, rowsResult] = await Promise.all([
        ...
        listBikeBlueprintMaintenanceParts(token, blueprintId, page, {
  ...
  const handleAssignExisting = async () => {
    try {
      ...
      if (modalMode === "single" && selectedPartIds[0]) {
        await assignMaintenancePartToBikeBlueprint(token, blueprintId, { maintenance_part_id: selectedPartIds[0] });
      } else if (modalMode === "bulk" && selectedPartIds.length > 0) {
        await assignMaintenancePartToBikeBlueprint(token, blueprintId, { maintenance_part_ids: selectedPartIds });
      }
  ...
  const handleCreateAndAssign = async (formData: Record<string, unknown>) => {
    try {
      ...
      const payload: CreateMaintenancePartPayload = {
  ...
  const handleRemoveAssignment = async (sparePartId: number) => {
    ...
    try {
      const token = getAuthToken();
      if (!token) throw new Error("Authentication required");
      await removeMaintenancePartFromBikeBlueprint(token, blueprintId, sparePartId);
      await loadAssignedRows();
    } catch (err) {
  ...
  return (
    <PageShell>
      <section className="overflow-hidden rounded-[1.75rem] border border-outline-variant/15 bg-surface-container-low shadow-ambient">
        <div className="space-y-4 p-5 md:p-6">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.4fr_0.9fr]">
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <span className="form-chip">Blueprint Maintenance Parts</span>
              ...
              {assignedRows.map((row) => (
                <tr key={row.id} className="border-b border-outline-variant/10 hover:bg-surface-container-low">
                  ...
                  <td className="px-4 py-3 text-on-surface">
                    {row.maintenance_part?.sale_price ?? 0} {row.maintenance_part?.sale_currency ?? "EGP"}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button
                      type="button"
                      onClick={() => handleRemoveAssignment(row.maintenance_part_id)}
      ...
      <BlueprintQuickAssignModal
        title={modalMode === "bulk" ? "Bulk Assign Maintenance Parts" : "Assign Existing Maintenance Part"}
        description={
          modalMode === "bulk"
      ...
      <EntityFormModal
        title="Create & Assign Maintenance Part"
