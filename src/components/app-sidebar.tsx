...
export function AppSidebar({
  ...
  return (
    <>
      ...
      >
        <div className="flex h-14 flex-shrink-0 items-center justify-between border-b border-outline-variant/20 px-3">
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
          {navSections.map((section, sectionIndex) => (
            <div key={`nav-section-${sectionIndex}`} className="space-y-0.5">
              {sectionIndex > 0 ? (
                <div
                  className={
                    isCollapsed
                      ? "my-1 border-t border-outline-variant/20"
                      : "my-1.5 border-t border-outline-variant/20"
                  }
                  aria-hidden
                />
              ) : null}
              ...
              {section.title ? (
                !isCollapsed ? (
                  <p className="px-3 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-on-surface-variant">
                    {section.title}
                  </p>
                ) : sectionIndex > 0 ? (
              ...
              {section.items.map((item, index) => {
                ...
                return (
                  <Link
                    ...
                    className={[
                      "flex items-center rounded-md px-3 py-1.5 text-sm font-medium transition-colors duration-200 ease-out",
                      isActive
        ...
        <div className="flex-shrink-0 border-t border-outline-variant/20 px-2 py-1.5">
          {!isCollapsed ? (
            <p className="mb-1.5 truncate text-[11px] text-on-surface-variant">
              Signed in: {userName}
            </p>
          ) : null}
          <button
            type="button"
            className="flex w-full items-center justify-center rounded-md bg-error-container px-3 py-1.5 text-xs font-semibold text-on-error-container hover:opacity-90"
            onClick={onLogout}
          >
