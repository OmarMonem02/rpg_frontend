...
import { ALL_ACTIONS, ALL_PAGE_PATHS, normalizePermissionMatrix } from "@/lib/permissions";
...
describe("PermissionsEditor", () => {
  ...
  it("selects all permissions for a page with the select all button", async () => {
    const user = userEvent.setup();
    const onSave = vi.fn().mockResolvedValue(undefined);
    ...
    await user.click(screen.getByRole("button", {
      name: "Dashboard select all permissions",
    }));
    await user.click(screen.getByRole("button", { name: "Save Permissions" }));

    const submittedMatrix = onSave.mock.calls[0][0];
    expect(submittedMatrix.dashboard).toEqual([...ALL_ACTIONS]);
  });
});
